...
class ForgetPasswordController extends Controller
{
    ...
    public function company_forget_password_submit(Request $request){
        ...
        $company_data->save(); // Cập nhật lại token

        $reset_link = url('reset_password/company/' . $token . '/' . $request->email);
    ...
    public function company_reset_password_submit(Request $request)
    {
        $request->validate([
         ...
         $company_data = Company::where('token', $request->token)->where('email', $request->email)->first();
            ...
            return redirect()->route('login')->with('success', 'Password has been reset successfully. You can now login to the system.');
    ...
    public function candidate_forget_password_submit(Request $request){
        ...
        $candidate_data->save(); // Cập nhật lại token

        $reset_link = url('reset_password/candidate/' . $token . '/' . $request->email);
        $subject = 'Reset Password';
        $message = 'Please click on the following link: <br>';
        $message .= '<a href="' . $reset_link . '">Click here</a>';
