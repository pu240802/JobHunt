...
class CompanyListingController extends Controller
{
    public function index(Request $request){
       ...
       $companies = Company::withCount('rJob')->with('rCompanyLocation','rCompanyIndustry','rCompanySize')->orderBy('id', 'desc');
       ...
       $companies = $companies->paginate(7)->withQueryString();
       ...
       'company_locations','company_sizes',
       'form_name','form_industry','form_location','form_size','form_founded','advertisement_data','other_page_item'));
    ...
    public function detail($id){
        ...
        $company_single = Company:: with('rCompany','rCompanyIndustry','rCompanyLocation','rCompanySize')
        ->where('id', $id)->first();
        ...
        if (CompanyPhoto::where('company_id', $company_single->id)->exists()) {
            ...
            $company_photos = collect(); // Gán thành một mảng rỗng khi không có hình ảnh
        }
       ...
       if(CompanyVideo:: where('company_id',$company_single->id)->exists()){
        $company_videos = CompanyVideo::where('company_id',$company_single->id)->get();
            ...
            $company_videos = collect();
        }
        
        $jobs = Job:: with('rJobCategory','rJobLocation','rJobSalaryRange',
        'rJobType','rJobExperience','rJobGender')->
        where('company_id', $company_single->id)->get();
        $other_page_item = PageOtherItem::where('id',1)->first();
      
         return view('front.company',compact('company_single','company_videos','company_photos','jobs','other_page_item'));
     }
