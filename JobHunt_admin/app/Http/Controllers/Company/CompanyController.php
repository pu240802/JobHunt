...
class CompanyController extends Controller  
{
    ...
    public function photos(){
       ...
       //Check if a person has access to this page under the currently package
        $package_data = Package::where('id',$order_data->package_id)->first();
        
        if($package_data->total_allowed_photo == 0){
            return redirect()->back()->with('error','Your current package does not allowed to access the photo section.');
        }
    ...
    public function photos_submit(Request $request){
        ...
        if($package_data-> total_allowed_photo == $existing_photo_number){
            return redirect()->back()->with('error','Maximum number of allowed photos are upload. So you have to upgrade your package if you want to add more photos.');
        }
    ...
    public function videos(){
       ...
       //Check if a person has access to this page under the currently package
        ...
        if($package_data->total_allowed_video == 0){
            return redirect()->back()->with('error','Your current package does not allowed to access the video section.');
        }
    ...
    public function videos_submit(Request $request){
        ...
        if($package_data-> total_allowed_video == $existing_video_number){
            return redirect()->back()->with('error','Maximum number of allowed videos are upload. So you have to upgrade your package if you want to add more videos.');
        }
    ...
    public function jobs_create(){
        ...
        //Check if a person a buy a package
        $order_data = Order::where('company_id',Auth::guard('company')->user()->id)->where('currently_active',1)->first();
       if(!$order_data){
        return redirect()->back()->with('error','You must have to buy a package first to access this page.');
       }
       ...
       //Check if a person has access to this page under the currently package
        ...
        if($package_data->total_allowed_jobs == 0){
            return redirect()->back()->with('error','Your current package does not allowed to access the job section.');
        }
        //How many jobs this company
        ...
        return view('company.jobs_create', compact('job_categories','job_locations','job_types',
        'job_experiences','job_genders','job_salary_ranges'));
    }
    public function jobs_create_submit(Request $request ){

        $company = Auth::guard('company')->user();
        if(empty($company->logo) ||empty($company->company_name) || empty($company->person_name) || 
        empty($company->username) || empty($company->email) || empty($company->phone) || 
        empty($company->address) || empty($company->company_location_id) || empty($company->company_industry_id) || 
        empty($company->company_size_id) || empty($company->founded_on)) {
            return redirect()->route('company_edit_profile')->with('error', 'Please update company information before creating a job.');
        ...
        $order_data = Order::where('company_id',Auth::guard('company')->user()->id)->where('currently_active',1)->first();
        $package_data = Package::where('id',$order_data->package_id)->first();
        //How many job featured job this company    
        $total_featured_jobs = Job::where('company_id',Auth::guard('company')->user()->id)->where('is_featured',1)->count();
    ...
    public function jobs_edit($id){
        ...
        $job_genders = JobGender::orderBy('id','asc')->get();
        $job_salary_ranges = JobSalaryRange::orderBy('id','asc')->get();
        return view('company.jobs_edit',compact('jobs_single','job_categories','job_locations','job_types',
        'job_experiences','job_genders','job_salary_ranges'));
    }
    ...
    public function candidate_applications(){
        $jobs = Job::with('rJobCategory','rJobLocation','rJobType','rJobGender', 
        'rJobExperience','rJobSalaryRange')->where('company_id', Auth::guard('company')->user()->id)->get();
    ...
    public function applicant_resume($id){
       ...
       $candidate_resumes = CandidateResume::where('candidate_id', $id)->get();
        ...
        return view('company.applicant_resume',compact('candidate_single','candidate_educations',
    'candidate_experiences','candidate_skills','candidate_awards','candidate_resumes'));
    }
